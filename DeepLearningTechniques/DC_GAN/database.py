...
class Database:
    ...
    def mon_data_to_db(self, epoch, train_gloss_mon, train_dloss_mon, train_time):
        '''
        ...
        cur.execute('insert into dcgan_log values(:train_log, :epoch, :log_time, :train_time, :train_gloss, :train_dloss)',
                    [self.train_log, epoch, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), train_time, train_gloss_mon, train_dloss_mon])
        self._conn.commit()
