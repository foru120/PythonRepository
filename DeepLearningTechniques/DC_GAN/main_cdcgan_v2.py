...
from DeepLearningTechniques.DC_GAN.cdcgan_v2 import CDCGAN
from DeepLearningTechniques.DC_GAN.database_cdc import Database
...
class Neuralnet:
    ...
    def _flag_setting(self):
        ...
        flags.DEFINE_string('trained_param_path',
                            'D:/05_source/PythonRepository/DeepLearningTechniques/DC_GAN/train_log/3th_test',
                            '훈련된 파라미터 값 저장 경로')
    ...
    def create_image(self, images, epoch, sess, name):
        ...
        for idx in range(0, len(images)):
            os.makedirs(os.path.join('gen_image', '5th_test', str(epoch), name), exist_ok=True)
            with open(os.path.join('gen_image', '5th_test', str(epoch), name, str(idx) + '.jpeg'), mode='wb') as f:
