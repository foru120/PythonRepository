    ...
    def __call__(self, inputs, training=False):
        ...
        with tf.variable_scope('g', reuse=self.reuse):
            with tf.variable_scope('reshape'):
                ...
                layer = tf.nn.relu(tf.layers.batch_normalization(layer, training=self.training), name='layer')
    ...
    def __call__(self, inputs, training=False, name=''):
        ...
        def convolution(layer, name, idx):
            with tf.variable_scope(name):
                ...
                layer = leaky_relu(tf.layers.batch_normalization(layer, training=training), name='layer')
            return layer
...
class DCGAN:
    def __init__(self, sess, batch_size=100, s_size=4, z_dim=100, img_size=64, learning_rate=0.0002,
                 g_depths=[1024, 512, 256, 128], d_depths=[64, 128, 256, 512]):
        self.sess = sess
    ...
    def _network(self):
        with tf.name_scope('initialize_step'):
            self.randomdata = tf.random_uniform(shape=[self.batch_size, self.z_dim], minval=-1.0, maxval=1.0)
        ...
        with tf.name_scope('dcgan_network'):
            generated_img = self.g(self.randomdata, training=self.training)
            g_outputs = self.d(generated_img, training=self.training, name='g')
        ...
        g_opt = tf.train.AdamOptimizer(learning_rate=self.learning_rate*10, beta1=0.5)
    ...
    def train(self, traindata):
        return self.sess.run([self.g_losses, self.d_losses, self.op],
                             feed_dict={self.traindata: traindata, self.training: True})

    def generate(self):
        return self.sess.run(self.g(tf.random_uniform(shape=[self.batch_size, self.z_dim], minval=-1.0, maxval=1.0), training=True))
