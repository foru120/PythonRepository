...
class Neuralnet:
    ...
    def _flag_setting(self):
        ...
        flags.DEFINE_string('trained_param_path',
                            'D:/Source/PythonRepository/DeepLearningTechniques/DC_GAN/gogh_train_log',
                            '훈련된 파라미터 값 저장 경로')
    ...
    def create_image(self, images, epoch, sess):
        images = tf.image.convert_image_dtype(tf.div(tf.add(images[np.random.permutation(self._FLAGS.batch_size)[:10]], 1.0), 2.0), tf.uint8)
        ...
        for idx in range(0, len(images)):
            ...
            with open('gogh_gen_image\\1th_test\\' + str(epoch) + '\\' + str(idx) + '.jpeg', mode='wb') as f:
        ...
        with tf.Session(config=self.config) as sess:
            ...
            while True:
                ...
                for step in range(num_train):
                    st = time.time()
                    batch_z = np.random.uniform(-1, 1, (self._FLAGS.batch_size, 100))
                    noise_z = np.random.randn(self._FLAGS.batch_size, 96, 128, 32)
                    train_data = sess.run(train_x)

                    step_d_loss = 0.
                    for _ in range(2):
                        d_loss, _ = dcgan.d_train(batch_z, noise_z, train_data)
                        tot_d_loss += d_loss
                        step_d_loss += d_loss

                    step_g_loss = 0.
                    for _ in range(1):
                        g_loss, _ = dcgan.g_train(batch_z, noise_z)
                        tot_g_loss += g_loss
                        step_g_loss += g_loss
