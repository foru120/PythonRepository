...
class Model():
    ...
    def _build_graph(self):
        with tf.name_scope('initialize_scope'):
            ...
            reshape_x = tf.reshape(self.X, shape=[-1, 32, 32, 1])
        ...
        def conv(l, kernel, channel, stride):
            return conv2d(inputs=l, num_outputs=channel, kernel_size=kernel, stride=stride, padding='SAME',
                          weights_initializer=variance_scaling_initializer(), biases_initializer=None, weights_regularizer=l2_regularizer(1e-4))
        ...
        def add_layer(name, l):
            with tf.variable_scope(name):
                ...
                # # c = self.parametric_relu(c, 'basic_2')
        ...
        def dense_net():
            ...
            logits = fully_connected(inputs=l, num_outputs=10, activation_fn=None,
                                     weights_initializer=variance_scaling_initializer(), weights_regularizer=l2_regularizer(1e-4))
