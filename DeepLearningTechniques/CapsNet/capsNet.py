...
from DeepLearningTechniques.CapsNet.utils import get_batch_data
from DeepLearningTechniques.CapsNet.capsLayer import CapsConv

class CapsNet(object):
    def __init__(self, is_training=True):
        self.graph = tf.Graph()  # 그룹으로 실행되는 operation 들의 집합(TensorFlow 는 Graph 라는 단위로 operation 을 수행)
        with self.graph.as_default():  # 모든 built op 들이 기본 전역 tf.Graph 인스턴스와 연관이 있다는 것을 의미
            if is_training:
                self.X, self.Y = get_batch_data()  # self.X : [batch_size, 28, 28, 1], self.Y : [batch_size, 10]
                self.build_arch()
                self.loss()
                ...
                self.train_op = self.optimizer.minimize(self.total_loss, global_step=self.global_step)  # global_step : 변수가 갱신된 후 1씩 증가하는 선택적 변수
            else:
    ...
    def build_arch(self):
        with tf.variable_scope('Conv1_layer'):
            # Conv1, input: [batch_size, 28, 28, 1], output: [batch_size, 20, 20, 256]
        ...
        with tf.variable_scope('Masking'):
            ...
            argmax_idx = tf.argmax(self.softmax_v, axis=1)
            argmax_idx = tf.cast(argmax_idx, tf.int32)
