...
from p02_cnn_image_classification.Cifar10.ensemble_cnn_model_bn_WT import Model
...
with tf.Session() as sess:
    ...
    while True:
        ...
        if len(early_stopping_list) >= 2:
            ...
            if diff > 2:
                print('Epoch: ', '%04d' % (epoch+1), 'cost =', avg_cost_list, ' - ', diff, ',', epoch_etime-epoch_stime)
        ...
        epoch += 1
        print('Epoch: ', '%04d' % (epoch), 'cost =', avg_cost_list, ' - ', diff, ',', epoch_etime-epoch_stime)
...
with tf.Session() as sess:
    ...
    # saver.restore(sess, 'log/epoch_17.ckpt')
