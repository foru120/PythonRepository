...
from DeepLearningTechniques.DenseNet.densenet_m import Model
import DeepLearningTechniques.DenseNet.cifar10_pickle as cifar10
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        if validation_loss < best_loss_val:
            ...
            saver.save(sess, 'train_log/densenet_cifar10_v7.ckpt')
        else:
...
def restore_and_test():
    ...
    with tf.Session(config=config) as sess:
        ...
        sess.run(tf.global_variables_initializer())
        saver = tf.train.Saver()
        saver.restore(sess, 'train_log/densenet_cifar10_v6.ckpt')
