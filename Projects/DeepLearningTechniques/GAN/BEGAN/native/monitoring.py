...
class Monitoring:
    ...
    def _mon_data_from_db(self):
        ...
        cur.execute('select train_type, acc, loss from cifar_train_log where model = :model and seq = :seq order by train_type, epoch',
                    [self.model, self.seq])
...
mon = Monitoring(model='mobilenet_v2', seq=2)