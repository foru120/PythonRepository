            ...
            for i in range(self.num_units):
                ...
                with tf.variable_scope('ConvUnit_' + str(i)):
                    caps_i = tf.contrib.layers.conv2d(input,
                                                      ...
                                                      self.stride,
                                                      padding="VALID")
                    caps_i = tf.reshape(caps_i, shape=(cfg.batch_size, -1, 1, 1))  # -1 : width(6) * height(6) * self.numoutputs
...
def capsule(input, b_IJ, idx_j):
    ...
    with tf.variable_scope('routing'):
        w_initializer = np.random.normal(size=[1, 1152, 8, 16], scale=0.01)  # scale 이 0.01 인 가우시안 정규분포 값 생성
        W_Ij = tf.Variable(w_initializer, dtype=tf.float32)
        # repeat W_Ij with batch_size times to shape [batch_size, 1152, 8, 16]
        W_Ij = tf.tile(W_Ij, [cfg.batch_size, 1, 1, 1])  # 입력된 W_lj 를 각 차원에 따라 반복적으로 추가하는 함수
        ...
        for r_iter in range(cfg.iter_routing):
            ...
            b_Il, b_Ij, b_Ir = tf.split(b_IJ, size_splits, axis=2)  # axis 에 지정된 축에 대해 분할 작업 수행
            print(b_Il.get_shape(), b_Ij.get_shape(), b_Ir.get_shape())
            c_Il, c_Ij, b_Ir = tf.split(c_IJ, size_splits, axis=2)
            assert c_Ij.get_shape() == [1, 1152, 1, 1]

            s_j = tf.multiply(c_Ij, u_hat)  # 같은 위치에 있는 값들끼리 곱셈 연산 수행
            ...
            v_j = squash(s_j)
            assert v_j.get_shape() == [cfg.batch_size, 1, 16, 1]
...
def squash(vector):
    ...
    '''
    vec_abs = tf.sqrt(tf.reduce_sum(tf.square(vector)))  # 모든 vector 의 합한 값을 구하는데, 음수가 있으므로 각각 제곱후 더한 다음 루트를 씌운다
    scalar_factor = tf.square(vec_abs) / (1 + tf.square(vec_abs))  # vec_abs 값을 0~1 사이의 factor 로 변환
    vec_squashed = scalar_factor * tf.divide(vector, vec_abs)  # scalar_factor 와 vec_abs 를 가지고 vector 의 크기를 0~1 사이로 설정
    return (vec_squashed)