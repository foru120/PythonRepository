...
class Monitoring:
    train_acc_list = deque(maxlen=300)
    train_loss_list = deque(maxlen=300)
    valid_acc_list = deque(maxlen=300)
    valid_loss_list = deque(maxlen=300)
    ...
    def __init__(self, dataset, model, seq):
        ...
        self.acc_subplot.set_xlim(0, 300)
        ...
        self.loss_subplot.set_xlim(0, 300)
    ...
    def _mon_data_from_db(self):
        ...
        cur.execute('select train_type, acc, loss from tiny_imagenet_train_log where model = :model and seq = :seq order by train_type, epoch',
                    [self.model, self.seq])
...
mon = Monitoring(dataset='tiny_imagenet', model='inception_resnet_v2', seq=8)