    ...
    def _build_graph(self):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope(name_or_scope='body_scope'):
                ...
                x, prev_x = self.normal_cell(x=x, prev_x=prev_x, num_filters=self.num_conv_filters * self.filters_multiplier, strides=2, name='normal_cell_2')

                #todo normal cell(3) / stride=1
                for idx in range(self.num_cells):
                    x, prev_x = self.normal_cell(x=x, prev_x=prev_x, num_filters=self.num_conv_filters * self.filters_multiplier, name='normal_cell_3_%s' % str(idx))

                #todo normal cell(4) / stride=2
                x, prev_x = self.normal_cell(x=x, prev_x=prev_x, num_filters=self.num_conv_filters * self.filters_multiplier ** 2, strides=2, name='normal_cell_4')
                ...
                for idx in range(self.num_cells):
                    x, prev_x = self.normal_cell(x=x, prev_x=prev_x, num_filters=self.num_conv_filters * self.filters_multiplier ** 2, name='normal_cell_5_%s' % str(idx))

                x = tf.nn.relu(x)
                x = tf.reduce_mean(x, axis=[1, 2], keepdims=False, name='global_avg_pool')
                x = self.dropout(inputs=x, rate=flags.FLAGS.dropout_rate, name='dropout')
                self.main_logits = self.dense(inputs=x, units=flags.FLAGS.image_class, name='main_logits')
            ...
            with tf.variable_scope(name_or_scope='output_scope'):
                ...
                self.prob = tf.nn.softmax(logits=self.main_logits, name='softmax')

                self.main_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.main_logits, labels=self.y, name='main_loss'))
                ...
                self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(self.main_logits, -1), self.y), dtype=tf.float32))
    ...
    def normal_cell(self, x, prev_x, num_filters, strides=1, name='normal_cell'):
        with tf.variable_scope(name_or_scope=name):
            ...
            with tf.variable_scope(name_or_scope='block_1'):
                x1_1 = self.separable_conv2d_block(inputs=h, filters=num_filters, kernel_size=7, strides=strides, name='sep_conv2d')
                x1_2 = tf.nn.max_pool(value=h, ksize=[1, 3, 3, 1], strides=[1, strides, strides, 1], padding='SAME', name='max_pool')
                x1 = tf.add(x1_1, x1_2, name='normal_add')

            with tf.variable_scope(name_or_scope='block_2'):
                x2_1 = self.separable_conv2d_block(inputs=h, filters=num_filters, kernel_size=5, strides=strides, name='sep_conv2d_1')
                x2_2 = self.separable_conv2d_block(inputs=h, filters=num_filters, strides=strides, name='sep_conv2d_2')
                x2 = tf.add(x2_1, x2_2, name='normal_add')

            with tf.variable_scope(name_or_scope='block_3'):
                x3_1 = self.separable_conv2d_block(inputs=x2, filters=num_filters, name='sep_conv2d')
                x3_2 = tf.nn.max_pool(value=h, ksize=[1, 3, 3, 1], strides=[1, strides, strides, 1], padding='SAME', name='avg_pool')
                x3 = tf.add(x3_1, x3_2, name='normal_add')

            with tf.variable_scope(name_or_scope='block_4'):
                x4 = self.separable_conv2d_block(inputs=prev_x, filters=num_filters, strides=strides, name='sep_conv2d')
                if strides == 2:
                    h = tf.nn.relu(features=x)
                    h = self.conv2d(inputs=h, filters=num_filters, strides=2, name='normal_conv')
                    h = self.batch_norm(inputs=h, act=None, name='normal_batch_norm')
                x4 = tf.add(h, x4, name='normal_add')

            with tf.variable_scope(name_or_scope='block_5'):
                x5_1 = self.separable_conv2d_block(inputs=prev_x, filters=num_filters, kernel_size=5, strides=strides, name='sep_conv2d_1')
                x5_2 = tf.nn.max_pool(value=prev_x, ksize=[1, 3, 3, 1], strides=[1, strides, strides, 1], padding='SAME', name='avg_pool')
                x5 = tf.add(x5_1, x5_2, name='normal_add')

            out_x = tf.concat([x1, x2, x3, x4, x5], axis=-1, name='normal_concat')
