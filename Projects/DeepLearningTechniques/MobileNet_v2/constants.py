...
flags = tf.app.flags
flags.DEFINE_string('data_path', 'G:/04_dataset/gelontoxon', '데이터 셋 저장 경로')
flags.DEFINE_string('trained_param_path',
                    'D:/Source/PythonRepository/DeepLearningTechniques/MobileNet_v2/train_log/1th_test',
                    '훈련된 파라미터 값 저장 경로')
flags.DEFINE_string('deploy_log_dir',
                    'D:/Source/PythonRepository/DeepLearningTechniques/MobileNet_v2/deploy_log/1th_test',
                    'Model Deploy 시 사용될 체크포인트 파일 저장 경로')

flags.DEFINE_integer('epochs', 500, '훈련 시 에폭 수')
