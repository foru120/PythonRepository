...
class Model:
    ...
    def _build_graph(self):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope(name_or_scope='body_scope'):
                x = self.conv2d(inputs=self.x, filters=self.num_conv_filters, kernel_size=3, strides=1, act=tf.nn.relu, name='stem_conv2d')
                x = self.batch_norm(inputs=x, act=None, name='stem_batch_norm')

                prev_x = None

                #todo normal cell(1) / stride=1
                for idx in range(self.num_cells):
                    x, prev_x = self.normal_cell(x=x, prev_x=prev_x, num_filters=self.num_conv_filters, name='normal_cell_1_%s' % str(idx))

                #todo normal cell(2) / stride=2
                x, prev_x = self.reduction_cell(x=x, prev_x=prev_x, num_filters=self.num_conv_filters * self.filters_multiplier, name='reduction_cell_1')

                #todo normal cell(3) / stride=1
                for idx in range(self.num_cells):
                    x, prev_x = self.normal_cell(x=x, prev_x=prev_x, num_filters=self.num_conv_filters * self.filters_multiplier, name='normal_cell_2_%s' % str(idx))

                #todo normal cell(4) / stride=2
                x, prev_x = self.reduction_cell(x=x, prev_x=prev_x, num_filters=self.num_conv_filters * self.filters_multiplier ** 2, name='reduction_cell_2')

                #todo auxiliary branch
                self.aux_logits = self.auxiliary_branch(x=x, name='auxiliary_branch')

                #todo normal cell(5) / stride=1
                for idx in range(self.num_cells):
                    x, prev_x = self.normal_cell(x=x, prev_x=prev_x, num_filters=self.num_conv_filters * self.filters_multiplier ** 2, name='normal_cell_3_%s' % str(idx))
            ...
            with tf.variable_scope(name_or_scope='output_scope'):
                ...
                self.loss = self.main_loss + self.aux_loss
    ...
    def adjust_block(self, x, prev_x, num_filters, name='adjust_block'):
        def skip_connection(x, num_filters, strides, name='adjust_reduction_block'):
            with tf.variable_scope(name_or_scope=name):
                ...
                path1 = self.conv2d(inputs=path1, filters=int(num_filters / 2), name='path1_conv2d')
                ...
                path2 = self.conv2d(inputs=path2, filters=int(num_filters / 2), name='path2_conv2d')
    ...
    def normal_cell(self, x, prev_x, num_filters, strides=1, name='normal_cell'):
        with tf.variable_scope(name_or_scope=name):
            ...
            with tf.variable_scope(name_or_scope='block_1'):
                x1_1 = tf.nn.avg_pool(value=prev_x, ksize=[1, 3, 3, 1], strides=[1, strides, strides, 1], padding='SAME', name='avg_pool')
                x1_2 = tf.nn.max_pool(value=prev_x, ksize=[1, 3, 3, 1], strides=[1, strides, strides, 1], padding='SAME', name='max_pool')
                x1 = tf.add(x1_1, x1_2, name='normal_add')

            with tf.variable_scope(name_or_scope='block_2'):
                x2 = tf.nn.avg_pool(value=h, ksize=[1, 3, 3, 1], strides=[1, strides, strides, 1], padding='SAME', name='avg_pool')
                x2 = tf.add(x2, prev_x, name='normal_add')

            with tf.variable_scope(name_or_scope='block_3'):
                x3_1 = self.separable_conv2d_block(inputs=x1, filters=num_filters, kernel_size=5, name='sep_conv2d_1')
                x3_2 = self.separable_conv2d_block(inputs=h, filters=num_filters, name='sep_conv2d_2')
                x3 = tf.add(x3_1, x3_2, name='normal_add')

            with tf.variable_scope(name_or_scope='block_4'):
                x4 = self.separable_conv2d_block(inputs=x1, filters=num_filters, name='sep_conv2d')
                x4 = tf.add(h, x4, name='normal_add')

            with tf.variable_scope(name_or_scope='block_5'):
                x5_1 = tf.nn.avg_pool(value=x3, ksize=[1, 3, 3, 1], strides=[1, strides, strides, 1], padding='SAME', name='avg_pool')
                x5_2 = self.separable_conv2d_block(inputs=prev_x, filters=num_filters, name='sep_conv2d')
                x5 = tf.add(x5_1, x5_2, name='normal_add')

            out_x = tf.concat([x2, x4, x5], axis=-1, name='normal_concat')
    ...
    def reduction_cell(self, x, prev_x, num_filters, name='reduction_cell'):
        with tf.variable_scope(name_or_scope=name):
            ...
            with tf.variable_scope(name_or_scope='block_1'):
                x1_1 = tf.nn.avg_pool(value=prev_x, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='avg_pool')
                x1_2 = self.separable_conv2d_block(inputs=h, filters=num_filters, strides=2, name='sep_conv2d')
                x1 = tf.add(x1_1, x1_2, name='reduction_add')

            with tf.variable_scope(name_or_scope='block_2'):
                x2_1 = tf.nn.max_pool(value=h, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='max_pool_1')
                x2_2 = tf.nn.max_pool(value=prev_x, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='max_pool_2')
                x2 = tf.add(x2_1, x2_2, name='reduction_add')

            with tf.variable_scope(name_or_scope='block_3'):
                x3_1 = tf.nn.max_pool(value=prev_x, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='max_pool')
                x3_2 = self.separable_conv2d_block(inputs=x1, kernel_size=7, filters=num_filters, name='sep_conv2d_2')
                x3 = tf.add(x3_1, x3_2, name='reduction_add')

            with tf.variable_scope(name_or_scope='block_4'):
                x4_1 = self.separable_conv2d_block(inputs=prev_x, filters=num_filters, kernel_size=7, strides=2, name='sep_conv2d')
                x4_2 = tf.nn.avg_pool(value=h, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='avg_pool')
                x4 = tf.add(x4_1, x4_2, name='reduction_add')

            with tf.variable_scope(name_or_scope='block_5'):
                x5_1 = self.separable_conv2d_block(inputs=x2, filters=num_filters, name='sep_conv2d')
                x5_2 = self.conv2d(inputs=prev_x, filters=num_filters, kernel_size=[7, 1], strides=2, name='factorize_conv2d_1')
                x5_2 = self.conv2d(inputs=x5_2, filters=num_filters, kernel_size=[1, 7], name='factorize_conv2d_2')
                x5 = tf.add(x5_1, x5_2, name='reduction_add')

            x = tf.concat([x3, x4, x5], axis=-1, name='reduction_concat')
    ...
    def batch_norm(self, inputs, act=tf.nn.relu6, name='batch_norm_layer'):
        ...
        '''
        with tf.variable_scope(name_or_scope=name):
            return tf.contrib.layers.batch_norm(inputs=inputs, decay=0.997, center=True, scale=True, fused=True,
    ...
    def separable_conv2d(self, inputs, filters, kernel_size=3, strides=1, padding='SAME', depth_multiplier=1, name=None):
        return slim.separable_conv2d(inputs=inputs, num_outputs=filters, kernel_size=kernel_size, activation_fn=tf.identity,
                                     weights_initializer=self.kernel_initializer, weights_regularizer=self.kernel_regularizer,
