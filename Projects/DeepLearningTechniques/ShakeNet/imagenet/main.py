...
from Projects.DeepLearningTechniques.ShakeNet.imagenet.data_loader import DataLoader
from Projects.DeepLearningTechniques.ShakeNet.imagenet.model import Model
from Projects.DeepLearningTechniques.ShakeNet.imagenet.database import Database
from Projects.DeepLearningTechniques.ShakeNet.imagenet.cam import GradCAM

class Trainer:

    def __init__(self, seq, is_db_logging, is_ckpt_logging, is_cfm_logging, is_roc_logging, is_tb_logging, name):
        self.seq = seq
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            #todo Model 객체 생성
            train_model = Model(sess=sess, width=flags.FLAGS.image_width, height=flags.FLAGS.image_height, channel=flags.FLAGS.image_channel,
                          lr=flags.FLAGS.learning_rate, dr=flags.FLAGS.dropout_rate,
                          is_training=True, is_tb_logging=self.is_tb_logging, name=self.name)

            valid_model = Model(sess=sess, width=flags.FLAGS.image_width, height=flags.FLAGS.image_height, channel=flags.FLAGS.image_channel,
                          lr=flags.FLAGS.learning_rate, dr=flags.FLAGS.dropout_rate,
                          is_training=False, is_tb_logging=self.is_tb_logging, name=self.name)
            ...
            for epoch in range(1, flags.FLAGS.epoch + 1):
                ...
                if self.is_ckpt_logging:
                    self._train_saver.save(sess, os.path.join(flags.FLAGS.train_log_path, str(self.seq), 'train_imagenet'), global_step=epoch)
                ...
                if self.is_db_logging:
                    self.db.mon_data_to_db(self.name, 'train', self.seq, epoch, train_acc_per_epoch, train_loss_per_epoch, (epoch_train_et - epoch_train_st))
                    self.db.mon_data_to_db(self.name, 'validation', self.seq, epoch, valid_acc_per_epoch, valid_loss_per_epoch, (epoch_valid_et - epoch_valid_st))
                ...
                if earlystop_patience >= flags.FLAGS.earlystop_patience:
                    ...
                    self.roc_curve(labels=np.asarray(valid_label_per_epoch).flatten(),
                                   probs=np.asarray(valid_prob_per_epoch).reshape((-1, flags.FLAGS.image_class)),
                                   epoch=epoch)
                    break
    ...
    def test(self):
        ...
        with tf.Session(config=config) as sess:
            test_model = Model(sess=sess, width=flags.FLAGS.image_width, height=flags.FLAGS.image_height, channel=flags.FLAGS.image_channel,
                          lr=flags.FLAGS.learning_rate, dr=flags.FLAGS.dropout_rate,
                          is_training=False, is_tb_logging=False, name=self.name)
            ...
            #todo Database logging
            if self.is_db_logging:
                self.db.mon_data_to_db(self.name, 'test', self.seq, 1, tot_test_acc, tot_test_loss, (tot_test_et - tot_test_st))

            #todo Checkpoint file logging
            if self.is_ckpt_logging:
                self._test_saver.save(sess, os.path.join(flags.FLAGS.deploy_log_path, str(self.seq), 'test_imagenet'))
    ...
    def visualization(self, sample_per_class):
        ...
        with tf.Session() as sess:
            ...
            model = Model(sess=sess, width=flags.FLAGS.image_width, height=flags.FLAGS.image_height, channel=flags.FLAGS.image_channel,
                          lr=flags.FLAGS.learning_rate, dr=flags.FLAGS.dropout_rate,
                          is_training=False, is_tb_logging=False, name=self.name)
