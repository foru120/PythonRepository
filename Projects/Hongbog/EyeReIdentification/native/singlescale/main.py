...
from Projects.Hongbog.EyeReIdentification.native.singlescale.model import Model
from Projects.Hongbog.EyeReIdentification.native.singlescale.dataloader import DataLoader
...
class Neuralnet:
    ...
    def train(self):
        ...
        train_right_1, train_right_2, train_right_3, train_right_4, train_right_5, train_right_6, \
        train_left_1, train_left_2, train_left_3, train_left_4, train_left_5, train_left_6 = self.loader.train_loader()
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(1, flags.FLAGS.epochs+1):
                ...
                for step in range(1, train_num+1):
                    ...
                    train_data = sess.run([train_right_1, train_right_2, train_right_3, train_right_4, train_right_5, train_right_6,
                                               train_left_1, train_left_2, train_left_3, train_left_4, train_left_5, train_left_6])
                    train_right_batch_ori_x, train_right_batch_query_x, train_right_batch_y = np.concatenate([right_data[0] for right_data in train_data[:6]]), \
                                                                                              np.concatenate([right_data[1] for right_data in train_data[:6]]), \
                                                                                              np.concatenate([right_data[2] for right_data in train_data[:6]])
                    train_left_batch_ori_x, train_left_batch_query_x, train_left_batch_y = np.concatenate([left_data[0] for left_data in train_data[6:]]), \
                                                                                           np.concatenate([left_data[1] for left_data in train_data[6:]]), \
                                                                                           np.concatenate([left_data[2] for left_data in train_data[6:]])
                    st = time.time()
                    step_train_right_acc, step_train_right_loss = [], []
                    for idx in range(0, flags.FLAGS.batch_size*6, flags.FLAGS.batch_size):
                        train_right_acc, train_right_loss, _ = right_model.train(ori_X=train_right_batch_ori_x[idx:idx+flags.FLAGS.batch_size],
                                                                                 query_X=train_right_batch_query_x[idx:idx+flags.FLAGS.batch_size],
                                                                                 y=train_right_batch_y[idx:idx+flags.FLAGS.batch_size])
                    ...
                    for idx in range(0, flags.FLAGS.batch_size*6, flags.FLAGS.batch_size):
                        train_left_acc, train_left_loss, _ = left_model.train(ori_X=train_left_batch_ori_x[idx:idx+flags.FLAGS.batch_size],
                                                                              query_X=train_left_batch_ori_x[idx:idx+flags.FLAGS.batch_size],
                                                                              y=train_left_batch_y[idx:idx + flags.FLAGS.batch_size])
