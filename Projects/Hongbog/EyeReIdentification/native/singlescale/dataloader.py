...
class DataLoader:
    ...
    def train_init(self):
        ...
        for key in train_right_path.keys():
            ...
            for idx in np.arange(0, 1200, 2, dtype=np.int32):
    ...
    def train_normal(self, x, y):
        with tf.variable_scope('train_normal'):
            ...
            ori_x = tf.image.decode_png(ori_x, channels=1, name='decode_img')
            ori_x = tf.image.resize_images(ori_x, size=DataLoader.IMG_SIZE, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            ori_x = self.tf_equalize_histogram(ori_x)
            ori_x = tf.divide(tf.cast(ori_x, tf.float32), 255.)

            query_x = tf.read_file(x[1])
            query_x = tf.image.decode_png(query_x, channels=1, name='decode_img')
            query_x = tf.image.resize_images(query_x, size=DataLoader.IMG_SIZE, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            query_x = self.tf_equalize_histogram(query_x)
            query_x = tf.divide(tf.cast(query_x, tf.float32), 255.)
        return ori_x, query_x, y

    def train_crop(self, x, y):
        with tf.variable_scope('train_crop'):
            ori_x = tf.read_file(x[0])
            ori_x = tf.image.decode_png(ori_x, channels=1, name='decode_img')
            ori_x = tf.image.resize_images(ori_x, size=(78, 170), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            ori_x = tf.random_crop(value=ori_x, size=(DataLoader.IMG_SIZE[0], DataLoader.IMG_SIZE[1], 1))
            ori_x = self.tf_equalize_histogram(ori_x)
            ori_x = tf.divide(tf.cast(ori_x, tf.float32), 255.)
            ...
            query_x = tf.random_crop(value=query_x, size=(DataLoader.IMG_SIZE[0], DataLoader.IMG_SIZE[1], 1))
            query_x = self.tf_equalize_histogram(query_x)
            query_x = tf.divide(tf.cast(query_x, tf.float32), 255.)
        return ori_x, query_x, y

    def train_brightness(self, x, y):
        with tf.variable_scope('train_brightness'):
            ori_x = tf.read_file(x[0])
            ori_x = tf.image.decode_jpeg(ori_x, channels=3, name='decode_img')
            ori_x = tf.image.resize_images(ori_x, size=DataLoader.IMG_SIZE, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            ...
            query_x = tf.read_file(x[1])
            query_x = tf.image.decode_jpeg(query_x, channels=3, name='decode_img')
            query_x = tf.image.resize_images(query_x, size=DataLoader.IMG_SIZE, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    def train_contrast(self, x, y):
        with tf.variable_scope('train_contrast'):
            ori_x = tf.read_file(x[0])
            ori_x = tf.image.decode_jpeg(ori_x, channels=3, name='decode_img')
            ori_x = tf.image.resize_images(ori_x, size=DataLoader.IMG_SIZE, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    def train_hue(self, x, y):
        with tf.variable_scope('train_hue'):
            ori_x = tf.read_file(x[0])
            ori_x = tf.image.decode_jpeg(ori_x, channels=3, name='decode_img')
            ori_x = tf.image.resize_images(ori_x, size=DataLoader.IMG_SIZE, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    def train_saturation(self, x, y):
        with tf.variable_scope('train_saturation'):
            ori_x = tf.read_file(x[0])
            ori_x = tf.image.decode_jpeg(ori_x, channels=3, name='decode_img')
            ori_x = tf.image.resize_images(ori_x, size=DataLoader.IMG_SIZE, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    def train_loader(self):
        with tf.variable_scope('train_loader'):
            ...
            right_normal_batch = self.dataset_batch_loader(right_dataset, self.train_normal, name='right_normal_loader')
            right_crop_batch = self.dataset_batch_loader(right_dataset, self.train_crop, name='right_crop_loader')
            right_brightness_batch = self.dataset_batch_loader(right_dataset, self.train_brightness, name='right_brightness_loader')
            right_contrast_batch = self.dataset_batch_loader(right_dataset, self.train_contrast, name='right_contrast_loader')
            right_hue_batch = self.dataset_batch_loader(right_dataset, self.train_hue, name='right_hue_loader')
            right_saturation_batch = self.dataset_batch_loader(right_dataset, self.train_saturation, name='right_saturation_loader')

            left_normal_batch = self.dataset_batch_loader(left_dataset, self.train_normal, name='left_normal_loader')
            left_crop_batch = self.dataset_batch_loader(left_dataset, self.train_crop, name='left_crop_loader')
            left_brightness_batch = self.dataset_batch_loader(left_dataset, self.train_brightness, name='left_brightness_loader')
            left_contrast_batch = self.dataset_batch_loader(left_dataset, self.train_contrast, name='left_contrast_loader')
            left_hue_batch = self.dataset_batch_loader(left_dataset, self.train_hue, name='left_hue_loader')
            left_saturation_batch = self.dataset_batch_loader(left_dataset, self.train_saturation, name='left_saturation_loader')
    ...
    def test_loader(self):
        with tf.variable_scope('test_loader'):
            ...
            right_normal_batch = self.dataset_batch_loader(right_dataset, self.test_normal, name='right_normal_loader')

            left_normal_batch = self.dataset_batch_loader(left_dataset, self.test_normal, name='left_normal_loader')
