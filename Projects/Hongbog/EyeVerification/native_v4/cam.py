...
class GradCAM:
    ...
    def build(self):
        ...
        with tf.variable_scope(self.name):
            ...
            one_hot = tf.one_hot(indices=tf.argmax(input=self.instance.prob, axis=-1), depth=self.num_classes)
            loss = tf.reduce_mean(tf.multiply(self.instance.logits, one_hot), axis=-1)
