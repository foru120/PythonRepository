...
class Model:
    ...
    def _build_graph(self):
        with tf.name_scope('initialize_scope'):
            self.low_res_X = tf.placeholder(dtype=tf.float32, shape=[None, 160, 60, 1], name='low_res_X')
            self.mid_res_X = tf.placeholder(dtype=tf.float32, shape=[None, 200, 80, 1], name='mid_res_X')
            self.high_res_X = tf.placeholder(dtype=tf.float32, shape=[None, 240, 100, 1], name='high_res_X')
        ...
        def _network():
            with tf.variable_scope(name_or_scope=self.name):
                ...
                with tf.variable_scope('low_resolution_network'):
                    ...
                    low_layer.outputs = tf.image.resize_nearest_neighbor(low_layer.outputs, (8, 4), name='resize')  # (8, 4)
                ...
                with tf.variable_scope('middle_resolution_network'):
                    ...
                    mid_layer.outputs = tf.image.resize_nearest_neighbor(mid_layer.outputs, (8, 4), name='resize')  # (8, 4)
                ...
                with tf.variable_scope('output_layer'):
                    layer = Conv2d(layer=layer, n_filter=2, filter_size=(1, 1), strides=(1, 1), act=tf.identity, name='logit')
