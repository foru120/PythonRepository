...
class Model(Layers):
    def __init__(self, sess, lr, is_training, name):
    ...
    def _build_graph(self):
        with tf.variable_scope(name_or_scope=self.name):
            with tf.variable_scope(name_or_scope='input_module'):
                self.low_res_X = tf.placeholder(dtype=tf.float32, shape=[None, 36, 200, 1], name='low_res_X')
                self.mid_res_X = tf.placeholder(dtype=tf.float32, shape=[None, 48, 260, 1], name='mid_res_X')
                self.high_res_X = tf.placeholder(dtype=tf.float32, shape=[None, 60, 320, 1], name='high_res_X')
            ...
            with tf.variable_scope(name_or_scope='low_res_module'):
                ...
                low_layer = tf.image.resize_nearest_neighbor(images=low_layer, size=(4, 20), name='resize')
            ...
            with tf.variable_scope(name_or_scope='mid_res_module'):
                ...
                mid_layer = tf.image.resize_nearest_neighbor(images=mid_layer, size=(4, 20), name='resize')
