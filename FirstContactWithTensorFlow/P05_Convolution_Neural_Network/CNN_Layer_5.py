...
conv_layer_output = [64, 64, 64, 128]  # 합성곱 계층 output 개수
last_layer_output = 2000  # 마지막 계층 output 개수
...
W_conv3 = weight_variable([*filter_cnt[2], conv_layer_output[1], conv_layer_output[2]])
...
#   - 64 개 가중치 행렬에 대한 편향 정의.
#   - 이전 계층의 출력 값의 크기를 채널의 수로 넘겨야 함.
W_conv4 = weight_variable([*filter_cnt[3], conv_layer_output[2], conv_layer_output[3]])
b_conv4 = bias_variable([conv_layer_output[3]], 'constant')

h_conv4 = tf.nn.relu(conv2d(h_pool3, W_conv4) + b_conv4)
h_pool4 = max_pool_2x2(h_conv4)
B, H, W, C = h_pool4.get_shape().as_list()
h_pool4_flat = tf.reshape(h_pool4, [-1, W*H*conv_layer_output[3]])  # softmax 함수는 이미지를 직렬화해서 벡터 형태로 입력해야 하므로 변환 수행.
...
W_fc1 = weight_variable([W*H*conv_layer_output[3], last_layer_output])
...
for i in range(1001):
    ...
    if i % 100 == 0:
        train_accracy = sess.run(accuracy, feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0})  # 평가시에는 드롭아웃 : 1.0
        print('step %d, training accuracy %g' %(i, train_accracy))
    sess.run(train_step, feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5})  # batch[0] : 입력 이미지에 대한 텐서, batch[1] : 정답 레이블 텐서, 학습시에는 드롭아웃 : 0.5
