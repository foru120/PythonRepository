...
class TesseractTrainer():
    ...
    def createTrainingFile(self, prefix):
        ...
        p = subprocess.Popen(["tesseract", prefix + ".tiff", prefix, "nobatch", "box.train"],
    ...
    def renameFiles(self):
        files = os.listdir(self.directory)  # CaptchaImages/ directory 밑에 있는 파일 리스트
        boxString = ""
        i = 0
        for fileName in files:
            if fileName.endswith(".box"):
                (root, ext) = os.path.splitext(fileName)  # 파일 이름과 확장자 분리
    ...
    # 약간이라도 어두운 부분은 검은색으로 변환
    def cleanImages(self):
        print("CLEANING IMAGES...")
        files = os.listdir(self.directory)  # CaptchaImages/ directory 에 해당하는 파일 리스트

        for fileName in files:
            if fileName.endswith("jpg") or fileName.endswith("jpeg") or fileName.endswith("png"):  # 해당 파일 이름이 jpg, jpeg, png 로 끝나는 경우
                image = Image.open(self.directory+"/"+fileName)
                #Set a threshold value for the image, and save
                image = image.point(lambda x: 0 if x < 250 else 255)  # 완전 흰색부분은 제외하고 나머지 부분을 검은색으로 변환
                (root, ext) = os.path.splitext(fileName)  # 파일 이름과 확장자를 분리
    ...
    def createFontFile(self):
        currentDir = os.getcwd()  # 현재 파일 경로
        # os.chdir(self.directory)  # 특정 directory로 현재 working directory 변경
        fname = self.directory+"/font_properties"
        with open(fname, 'w') as fout:  # fname = ScrapedImages/font_properties 파일 생성
            fout.write(self.fontName+" 0 0 0 0 0")
        # os.chdir(currentDir)  # 현재 파일 경로로 working directory 변경
