...
conv_layer_output = [64, 128, 128]  # 합성곱 계층 output 개수
last_layer_output = 1024  # 마지막 계층 output 개수
filter_cnt = [(5, 5), (3, 3), (3, 3)]  # 합성곱 계층 filter 개수
output_cnt = 10  # 출력 개수
...
B, H, W, C = h_pool3.get_shape().as_list()
W_fc1 = weight_variable([W*H*conv_layer_output[2], last_layer_output])
b_fc1 = bias_variable([last_layer_output], 'constant')

h_pool3_flat = tf.reshape(h_pool3, [-1, W*H*conv_layer_output[2]])  # softmax 함수는 이미지를 직렬화해서 벡터 형태로 입력해야 하므로 변환 수행.
