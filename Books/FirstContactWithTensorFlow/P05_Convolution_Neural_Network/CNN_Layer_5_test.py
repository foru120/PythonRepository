...
conv_layer_output = [32, 64, 128, 256]  # 합성곱 계층 output 개수
nn_layer_output = [1000, 500]  # 마지막 계층 output 개수
filter_cnt = [(3, 3), (3, 3), (3, 3), (3, 3)]  # 합성곱 계층 filter 개수
output_cnt = 10  # 출력 개수
training_epochs = 15  # epoch(mnist 데이터 셋을 전체 처리하는 단위) 개수
batch_size = 100  # 한번에 수행되는 학습 단위
learning_rate = 0.01
...
W_conv1 = weight_variable([*filter_cnt[0], 1, conv_layer_output[0]], 'w1')
...
#   - 윈도우 크기가 5 x 5 인 32 개 필터 생성.
#   - 32 개 가중치 행렬에 대한 편향 정의.
W_conv2 = weight_variable([*filter_cnt[1], conv_layer_output[0], conv_layer_output[1]], 'w2')
...
# ▣ 세 번째 은닉층
#  1. 합성곱 계층
#   - 윈도우 크기가 3 x 3 인 64 개의 필터 생성.
#   - 64 개 가중치 행렬에 대한 편향 정의.
#   - 이전 계층의 출력 값의 크기를 채널의 수로 넘겨야 함.
W_conv3 = weight_variable([*filter_cnt[2], conv_layer_output[1], conv_layer_output[2]], 'w3')
b_conv3 = bias_variable([conv_layer_output[2]], 'constant')
...
#   - 64 개 가중치 행렬에 대한 편향 정의.
#   - 이전 계층의 출력 값의 크기를 채널의 수로 넘겨야 함.
W_conv4 = weight_variable([*filter_cnt[3], conv_layer_output[2], conv_layer_output[3]], 'w4')
b_conv4 = bias_variable([conv_layer_output[3]], 'constant')

h_conv4 = tf.nn.relu(conv2d(h_pool3, W_conv4) + b_conv4)
# h_pool4 = max_pool_2x2(h_conv4)
# h_pool4 = tf.nn.dropout(h_pool4, keep_prob)
B, H, W, C = h_conv4.get_shape().as_list()
h_pool4_flat = tf.reshape(h_conv4, [-1, W*H*conv_layer_output[3]])  # softmax 함수는 이미지를 직렬화해서 벡터 형태로 입력해야 하므로 변환 수행.

# ▣ 첫번째 Fully Connected 은닉층
...
W_fc1 = weight_variable([W*H*conv_layer_output[3], nn_layer_output[0]], 'w5')
# b_fc1 = bias_variable([last_layer_output], 'He', 'b5')
b_fc1 = bias_variable([nn_layer_output[0]], 'constant')
h_fc1 = tf.nn.relu(tf.matmul(h_pool4_flat, W_fc1) + b_fc1)

# ▣ 드롭아웃
#  - 랜덤으로 노드를 삭제하여 입력과 출력 사이의 연결을 제거하는 기법.
#  - 모델이 데이터에 오버피팅 되는 것을 막아주는 역할.
#  - 학습시에는 dropout rate < 1, 평가 : 1
h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

# ▣ 마지막 은닉층
W_fc2 = weight_variable([nn_layer_output[0], nn_layer_output[1]], 'w6')
# b_fc1 = bias_variable([last_layer_output], 'He', 'b5')
b_fc2 = bias_variable([nn_layer_output[1]], 'constant')
...
W_fc3 = weight_variable([nn_layer_output[1], output_cnt], 'w7')
# b_fc2 = bias_variable([output_cnt], 'He', 'b6')
b_fc3 = bias_variable([output_cnt], 'constant')
...
cross_entropy = -tf.reduce_sum(y_ * tf.log(y_conv)) + 0.01*tf.reduce_sum(tf.square(W_fc2))
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        c, _ = sess.run([cross_entropy, train_step], feed_dict={x: batch_xs, y_: batch_ys, keep_prob: 0.5})  # 학습시에는 드롭아웃 : 0.5
