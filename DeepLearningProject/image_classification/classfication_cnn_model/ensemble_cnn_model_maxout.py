    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.name_scope('conv_layer1') as scope:
                self.W1 = tf.get_variable(name='W1', shape=[7, 7, 1, 20], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                self.b1 = tf.Variable(tf.constant(value=0.001, shape=[20]), name='b1')
                ...
                self.L1 = tf.nn.max_pool(value=self.L1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 120x120 -> 60x60
                self.L1 = tf.layers.dropout(inputs=self.L1, rate=self.dropout_rate, training=self.training)

            ############################################################################################################
            ## ▣ Convolution 계층 - 2
            ##  ⊙ 합성곱 계층 → filter: (5, 5), output: 40 개, 초기값: He
            ...
            with tf.name_scope('conv_layer2') as scope:
                self.W2 = tf.get_variable(name='W2', shape=[5, 5, 20, 40], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                self.b2 = tf.Variable(tf.constant(value=0.001, shape=[40]), name='b2')
            ...
            with tf.name_scope('conv_layer3') as scope:
                ...
                self.b3 = tf.Variable(tf.constant(value=0.001, shape=[80]), name='b3')
            ...
            with tf.name_scope('conv_layer4') as scope:
                ...
                self.b4 = tf.Variable(tf.constant(value=0.001, shape=[160]), name='b4')
            ...
            with tf.name_scope('fc_layer1') as scope:
                self.W_fc1 = tf.get_variable(name='W_fc1', shape=[4 * 4 * 320, 1800], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                self.b_fc1 = tf.Variable(tf.constant(value=0.001, shape=[1800], name='b_fc1'))
                ...
                self.L_fc1 = self.max_out(tf.matmul(self.L5, self.W_fc1) + self.b_fc1, 600)
            ...
            with tf.name_scope('fc_layer2') as scope:
                self.W_fc2 = tf.get_variable(name='W_fc2', shape=[600, 600], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                ...
                self.L_fc2 = self.max_out(tf.matmul(self.L_fc1, self.W_fc2) + self.b_fc2, 200)
            ...
            ##  ⊙ 활성화 함수 → Softmax
            ############################################################################################################
            self.W_out = tf.get_variable(name='W_out', shape=[200, 2], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
