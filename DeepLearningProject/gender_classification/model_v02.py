...
def he_initializer(shape):
    return tf.get_variable(shape=shape, dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())

def BN(input, training, name):
...
def conv1(input_data, training):  # 126x126 -> 63x63
    ...
    with tf.name_scope('conv_1'):
        W_conv1 = he_initializer([FLAGS.conv1_filter_size, FLAGS.conv1_filter_size, FLAGS.image_color, FLAGS.conv1_layer_size])
        ...
        h_conv1 = BN(input=h_conv1, training=training)
...
def conv2(input_data, training):  # 63x63 -> 32x32
    FLAGS.conv2_filter_size = 5
    FLAGS.conv2_layer_size = 64
    FLAGS.stride2 = 1

    with tf.name_scope('conv_2'):
        W_conv2 = he_initializer([FLAGS.conv2_filter_size, FLAGS.conv2_filter_size, FLAGS.conv1_layer_size, FLAGS.conv2_layer_size])
        ...
        h_conv2 = BN(input=h_conv2, training=training)
...
# cnn 3계층
def conv3(input_data, training):  # 32x32 -> 16x16
    ...
    with tf.name_scope('conv_3'):
        W_conv3 = he_initializer([FLAGS.conv3_filter_size, FLAGS.conv3_filter_size, FLAGS.conv2_layer_size, FLAGS.conv3_layer_size])
        ...
        h_conv3 = BN(input=h_conv3, training=training)
...
def conv4(input_data, training):  # 16x16 -> 8x8
    FLAGS.conv4_filter_size = 5
    FLAGS.conv4_layer_size = 128
    FLAGS.stride4 = 1

    with tf.name_scope('conv_4'):
        W_conv4 = he_initializer([FLAGS.conv4_filter_size, FLAGS.conv4_filter_size, FLAGS.conv3_layer_size, FLAGS.conv4_layer_size])
        ...
        h_conv4 = BN(input=h_conv4, training=training)
...
def fc1(input_data, training):
    ...
    with tf.name_scope('fc_1'):
        ...
        W_fc1 = he_initializer([input_layer_size, FLAGS.fc1_layer_size])
        ...
        h_fc1 = BN(input=h_fc1, training=training)
        h_fc1_relu = tf.nn.relu(h_fc1)

    return h_fc1_relu

# fully connected layer 2
def fc2(input_data, training):
    FLAGS.fc2_layer_size = 256

    with tf.name_scope('fc_2'):
        W_fc2 = he_initializer([FLAGS.fc1_layer_size, FLAGS.fc2_layer_size])
        ...
        h_fc2 = BN(input=h_fc2, training=training)
...
def final_out(input_data):
    with tf.name_scope('final_out'):
        W_fo = he_initializer([FLAGS.fc2_layer_size, FLAGS.num_classes])
...
# build cnn_graph !
def build_model(images, keep_prob, training):
    # define CNN network graph
    # output shape will be (*,48,48,16)
    r_cnn1 = conv1(images, training)  # convolutional layer 1
    # print("shape after cnn1 ", r_cnn1.get_shape())

    # output shape will be (*,24,24,32)
    r_cnn2 = conv2(r_cnn1, training)  # convolutional layer 2
    # print("shape after cnn2 :", r_cnn2.get_shape())

    # output shape will be (*,12,12,64)
    r_cnn3 = conv3(r_cnn2, training)  # convolutional layer 3
    # print("shape after cnn3 :", r_cnn3.get_shape())

    # output shape will be (*,6,6,128)
    # 3 or 4 ?
    r_cnn4 = conv4(r_cnn3, training)  # convolutional layer 4
    # print("shape after cnn4 :", r_cnn4.get_shape())

    # fully connected layer 1
    r_fc1 = fc1(r_cnn4, training)
    # print("shape after fc1 :", r_fc1.get_shape())

    # fully connected layer2
    r_fc2 = fc2(r_fc1, training)
...
def main(argv=None):
    ...
    prediction = build_model(reshape_images, keep_prob, training)
