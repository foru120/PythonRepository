...
class Model:
    ...
    def _build_graph(self):
        with tf.name_scope('initialize_scope'):
            self.X = tf.placeholder(dtype=tf.float32, shape=[None, 50, 100, 1], name='X_data')
        ...
        def _residual_block(input, n_filter, filter_size, strides, name):
            ...
            with tf.variable_scope(name):
                ...
                layer = Conv2d(layer=layer, n_filter=n_filter, filter_size=filter_size, strides=strides,
                               act=tf.identity, name='residual_f1')
                layer = _batch_norm(layer, act=tf.nn.elu, name='batch_norm_f2')
                layer = Conv2d(layer=layer, n_filter=n_filter, filter_size=filter_size, strides=strides,
                               act=tf.identity, name='residual_f2')
