...
class DataLoader:
    def __init__(self, batch_size, train_right_root_path, test_right_root_path,
                 train_left_root_path, test_left_root_path):
    ...
    def train_right_normal_data(self, x, y):
        with tf.variable_scope('train_right_normal_data'):
            ...
            x = tf.image.resize_images(x, size=(50, 100))
            x = tf.divide(x, 255.)
        return x, y
    ...
    def train_right_crop_data(self, x, y):
        with tf.variable_scope('train_right_crop_data'):
            ...
            x = tf.image.decode_png(x, channels=1, name='decode_img')
            x = tf.transpose(x, perm=[1, 0, 2])
            x = tf.image.resize_images(x, size=(60, 120))
            x = tf.random_crop(value=x, size=(50, 100, 1))
            x = tf.divide(x, 255.)
        return x, y
