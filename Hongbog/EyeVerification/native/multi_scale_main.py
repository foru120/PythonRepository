...
class Neuralnet:
    ...
    def __init__(self, is_train, save_type=None):
        ...
        if (self.is_train == True) and (save_type == 'db'):  # data loading
            # self.db = Database(FLAGS=flags.FLAGS, train_log=1)
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            right_model = Model(sess=sess, lr=flags.FLAGS.learning_rate, name='right')
            left_model = Model(sess=sess, lr=flags.FLAGS.learning_rate, name='left')

            print('>> Tensorflow session built. Variables initialized')
            sess.run(tf.global_variables_initializer())

            tf.train.write_graph(sess.graph_def, flags.FLAGS.trained_weight_path, 'graph.pbtxt')
            print('>> Graph saved')

            self._saver = tf.train.Saver()
            ckpt_st = tf.train.get_checkpoint_state(os.path.join(flags.FLAGS.trained_weight_path))
            ...
            for epoch in range(1, flags.FLAGS.epochs+1):
                ...
                os.makedirs(os.path.join(flags.FLAGS.trained_weight_path), exist_ok=True)
                self._saver.save(sess, os.path.join(flags.FLAGS.trained_weight_path, 'eye_verification_param'), global_step=epoch)

                ## PB File Save
                # builder = tf.saved_model.builder.SavedModelBuilder(os.path.join(flags.FLAGS.trained_weight_path, 'eye_verification_param'))
